...
async function resolveVendorName(vendorId, vendorName) {
  const provided = String(vendorName || '').trim();
  if (provided) return provided;
  const { rows } = await query(
    `SELECT vendor_name FROM vendor_metrics WHERE vendor_id = $1`,
    [vendorId]
  );
  return rows[0]?.vendor_name || null;
}
...
export async function registerVendorDownload({
  ...
  const resolvedName = await resolveVendorName(vendorId, vendorName);
  const displayName = vendorTemplateFilename(resolvedName, vendorId);
