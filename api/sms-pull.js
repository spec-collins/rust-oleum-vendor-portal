import { assertSmsPull } from '../lib/sms-auth.js';
...
/**
 * Spec Management System (SMS) pull API — stable pathnames for vendor uploads.
 *
 * Auth: x-sms-token / Authorization: Bearer / ?token=
 *       (SMS_PULL_TOKEN, else ADMIN_TOKEN)
 *
 * - GET                    → vendors + upload counts
 * - GET ?vendor_id=        → files for one vendor
 * - GET ?id= or ?pathname= → 302 short-lived private download
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('sms-pull failed:', err);
    return sendJson(res, 500, { ok: false, error: 'Could not list uploads for SMS pull.' });
  }
}
...
async function downloadOne(res, id, pathnameQ) {
  try {
    ...
    const location = await presignPrivateGet(row.pathname, { ttlMs: 15 * 60 * 1000 });
    res.statusCode = 302;
    ...
    console.error('sms-pull download failed:', err);
    return sendJson(res, 500, { ok: false, error: 'Could not prepare SMS download.' });
  }
}
